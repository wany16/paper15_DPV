...
create_ip -name input_arbiter_6_inputs -vendor NetFPGA -library NetFPGA -module_name input_arbiter_6_inputs_ip
set_property generate_synth_checkpoint false [get_files input_arbiter_6_inputs_ip.xci]
reset_target all [get_ips input_arbiter_6_inputs_ip]
generate_target all [get_ips input_arbiter_6_inputs_ip]

### IPG
create_ip -name ipg -vendor NetFPGA -library NetFPGA -module_name ipg_ip
set_property generate_synth_checkpoint false [get_files ipg_ip.xci]
reset_target all [get_ips ipg_ip]
generate_target all [get_ips ipg_ip]

### SPLT
create_ip -name splt -vendor NetFPGA -library NetFPGA -module_name splt_ip
set_property generate_synth_checkpoint false [get_files splt_ip.xci]
reset_target all [get_ips splt_ip]
generate_target all [get_ips splt_ip]

### MOD
create_ip -name mod -vendor NetFPGA -library NetFPGA -module_name mod_ip
set_property generate_synth_checkpoint false [get_files mod_ip.xci]
reset_target all [get_ips mod_ip]
generate_target all [get_ips mod_ip]

### DBG
#source ../hw/create_ip/nf_sume_sdnet.tcl   # only need this if sdnet_to_sume has fifo in wrapper
create_ip -name nf_sume_sdnet_dbg -vendor NetFPGA -library NetFPGA -module_name nf_sume_sdnet_dbg_ip
set_property generate_synth_checkpoint false [get_files nf_sume_sdnet_dbg_ip.xci]
reset_target all [get_ips nf_sume_sdnet_dbg_ip]
generate_target all [get_ips nf_sume_sdnet_dbg_ip]

### SWITCH PIPELINE (PPL)
#source ../hw/create_ip/nf_sume_sdnet.tcl   # only need this if sdnet_to_sume has fifo in wrapper
create_ip -name $::env(SWITCH_PPL) -vendor NetFPGA -library NetFPGA -module_name output_port_lookup_ip
...
### VER
#source ../hw/create_ip/nf_sume_sdnet.tcl   # only need this if sdnet_to_sume has fifo in wrapper
create_ip -name nf_sume_sdnet_ver -vendor NetFPGA -library NetFPGA -module_name nf_sume_sdnet_ver_ip
set_property generate_synth_checkpoint false [get_files nf_sume_sdnet_ver_ip.xci]
reset_target all [get_ips nf_sume_sdnet_ver_ip]
generate_target all [get_ips nf_sume_sdnet_ver_ip]
...
### PPL
#source ../hw/create_ip/nf_sume_sdnet.tcl   # only need this if sdnet_to_sume has fifo in wrapper
# create_ip -name nf_sume_sdnet_ppl -vendor NetFPGA -library NetFPGA -module_name nf_sume_sdnet_ppl_ip
# set_property generate_synth_checkpoint false [get_files nf_sume_sdnet_ppl_ip.xci]
# reset_target all [get_ips nf_sume_sdnet_ppl_ip]
# generate_target all [get_ips nf_sume_sdnet_ppl_ip]
