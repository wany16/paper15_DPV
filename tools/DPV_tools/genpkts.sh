...
if [[ "$1" == "-h" || "$1" == "--h" || "$1" == "-help" || "$1" == "--help" ]]; then
  ...
  echo "(-f) flags [8b value]: |na|-|na|-|na|-|C|-|S|-|M|-|R|-|T| ==> MORE INFO: (--hf)"
...
while true; do
  case "$1" in
    ...
    --hf) echo "";
      ...
      echo "";
      echo "|x|x|x|C|S|M|R|T| (DEFAULT: 00000000)";
      echo " 7 6 5 4 3 2 1 0";
      echo "";
      echo "[0:0] --> T: TIMESTAMP / else: OFF";
      echo "[1:1] --> R: RANDOM VALUES / else: CUSTOM VALUES ";
      echo "[2:2] --> M: TRAINING / else: TEST";
      echo "[3:3] --> S: SWAP / else: OFF";
      echo "[4:4] --> C: COUNTER / else OFF";
      echo "[5:5] --> X: not assigned";
      echo "[6:6] --> Y: not assigned";
      echo "[7:7] --> Z: not assigned";
      echo "";
      shift; shift; exit 0 ;;
    * ) break ;;
  esac
